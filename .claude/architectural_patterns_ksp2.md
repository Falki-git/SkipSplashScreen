...
the game's idioms. All type names verified by decompiling the shipped assemblies. This doc is
mod-agnostic; for the current mod's specifics see `mod_specifics.md`.
...
**Commonly useful messages:** `GameStateChangedMessage`, `GameStateEnteredMessage`/
`GameStateLeftMessage`, `GameLoadFinishedMessage`, `VesselChangedMessage`,
`MapViewEnteredMessage`/`MapViewLeftMessage`, `FlightViewEnteredMessage`/`FlightViewLeftMessage`,
`ObserverSOIChangedMessage`, `MapOrbitAddedMessage`/`MapOrbitRemovedMessage`.
...
**Existing modules to study before writing new ones** (in `KSP.Sim.impl` / `KSP.Modules`
...
`ResourceContainerGroup`, `ResourceDefinitionDatabase`. A module that consumes EC requests
...
`VesselComponent` is a central sim type for gameplay mods:
```csharp
...
- Provided members: `SWConfiguration`, `SWMetadata`, `CreateHarmonyAndPatchAll()`. For logging,
  ...
  — not `SWLogger`.
- Mod metadata is declared in `swinfo` (id, version, dependencies, `main_assembly`).
...
- Localization uses **I2.Loc** (`I2.Loc.*`) in the base game; SpaceWarp2/UitkForKsp2 provide
  ...
  files live under each mod's `Copied/localizations/` folder (shipped to `localizations/` in
  the built mod).
...
Stock UI lives in `KSP.UI` (202 types) and `KSP.UI.Binding`; map UI in `KSP.Map`. For a
map/surface overlay, expect to combine a UitkForKsp2 window with `MapProvider`/`KSP.Map` and
possibly a render texture overlay (see `Redux.WorldVis.PqsOverlay` for PQS terrain overlay
precedent).
...
`Transpiler`. Apply all patches in a mod via `CreateHarmonyAndPatchAll()` from the mod base
(`KerbalMod`/`GeneralMod`).
